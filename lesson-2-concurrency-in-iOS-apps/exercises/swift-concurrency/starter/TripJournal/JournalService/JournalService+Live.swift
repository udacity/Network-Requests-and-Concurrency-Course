...
class JournalServiceLive: JournalService {
    ...
    func register(username: String, password: String) async throws -> Token {
        // Create the register request using the createRegisterRequest method
        // Perform the network request using the performNetworkRequest method and return the token
    }

    func logIn(username: String, password: String) async throws -> Token {
        // Create the login request using the createLoginRequest method
        // Perform the network request using the performNetworkRequest method and return the token
    }
    ...
    private func performNetworkRequest(_ request: URLRequest) async throws -> Token {
        ...
        // Cast the response to HTTPURLResponse and check if the statusCode is 200
        // otherwise return NetworkError.badResponse
