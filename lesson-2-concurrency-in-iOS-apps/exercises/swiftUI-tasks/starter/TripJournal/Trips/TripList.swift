...
struct TripList: View {
    ...
    var body: some View {
        NavigationStack {
            content
                ...
                .sheet(item: $tripFormMode) { mode in
                    TripForm(mode: mode) {
                        // 1. Call fetchTrips asynchronously
                    }
                }
                .confirmationDialog(
                    "Log out?",
        ...
        // 2. Use the Task modifier to call fetchTrips asynchronously when the view appears
    }
        ...
        List {
            ForEach(trips) { trip in
                TripCell(
                    ...
                    edit: {},
                    delete: {
                        // 3.  Use Task to call deleteTrip asynchronously when deleting a trip
                    }
                )
            }
        }
        // 4. Use the refreshable modifier to call fetchTrips asynchronously when pulling to refresh
    }
