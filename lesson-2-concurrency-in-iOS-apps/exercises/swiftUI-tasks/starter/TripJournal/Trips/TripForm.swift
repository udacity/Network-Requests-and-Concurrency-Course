...
struct TripForm: View {
    ...
    var body: some View {
        NavigationView {
            ...
            // 1. Cancel any ongoing task when the view disappears to prevent unnecessary operations
        }
    }
    ...
    private func toolbar() -> some ToolbarContent {
        ...
        ToolbarItem(placement: .primaryAction) {
            Button("Save") {
                switch mode {
                case .add:
                    // TODO: Assign the task to the state property when the Save button is tapped to manage the asynchronous addTrip operation
                }
            }
        }
    }
